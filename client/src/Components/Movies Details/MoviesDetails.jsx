...
const MoviesDetails = () => {
  ...
  const fetchPost = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/movie/getMoviesById/${id}`
      );
      setMovie(response.data.movieById);

      console.log(response.data.movieById);
  ...
  const showtimesArrayString = movie.showtimes[0].replace(/'/g, '"');
  const showtimesArray = JSON.parse(showtimesArrayString);
  return (
    <div>
            ...
            <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
              ...
              <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-200 mb-5">
                <div className="flex ml-6 items-center">
                  <span className="mr-3">Show Times:</span>
              ...
              <div className="flex">
                ...
                <button className="flex ml-auto text-white bg-red-500 border-0 py-2 px-6 focus:outline-none hover:bg-red-600 rounded">
                  <Link to="/booktickets">Book Ticket</Link>
                </button>
